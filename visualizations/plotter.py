...
class NeuronPlotter():
    ...
    def plot_membrane_potential(self):
        ...
        plt.ylim(min(vs)-5, max(vs)+5)
        plt.xlim(0, t[-1])
        plt.xlabel('Time (ms)')
        plt.ylabel('Membrane Potential (mV)')
        plt.title(f'Hodgkin-Huxley Neuron Membrane Potential (I_ext = {self.neuron.I_ext} μA/cm²)')
        plt.grid(True)
        plt.legend()
        plt.show()
    ...
    def visualize_simulation(self, animate=False):
        ...
        if not animate:
            ...
            ax1.set_title(f'Hodgkin-Huxley Neuron Simulation (I_ext = {self.neuron.I_ext} μA/cm²)')
            ax1.grid(True)
    ...
    def plot_rate_constants(self, v_min=-70, v_max=50, num_points=1000):
        vs = np.linspace(v_min, v_max, num_points)

        fig, axs = plt.subplots(2, 2, figsize=(12, 8), sharex=True)
        fig.suptitle(f'Hodgkin-Huxley Rate Constants vs Membrane Potential (I_ext = {self.neuron.I_ext} μA/cm²)')
    ...
    def plot_open_channels(self):
        ...
        ax2.set_xlim(0, t[-1])
        ax2.set_ylim(0, max(np.max(I_K), np.max(I_Na)) * 1.1)
        ax2.set_ylabel('Conductance (mS/cm²)')
        ax2.legend()
        ax2.grid(True)
        ax2.set_title(f'Open Channel Conductance Over Time (I_ext = {self.neuron.I_ext} μA/cm²)')

        ax1.plot(t, v, label='Membrane Potential (V)', color='black')
        ax1.axhline(y=-55, color='r', linestyle='--', label='Threshold (-55 mV)')
        ax1.set_xlim(0, t[-1])
        ax1.set_ylim(min(v)-5, max(v)+5)
        ax1.set_xlabel('Time (ms)')
        ax1.set_ylabel('Membrane Potential (mV)')
        ax1.legend()
        ax1.grid(True)
        ax1.set_title(f'Membrane Potential Over Time (I_ext = {self.neuron.I_ext} μA/cm²)')
    ...
    def plot_gating_variables(self):
        results = self.neuron.results
        vs = results['v']
        ns = results['n']
        ms = results['m']
        hs = results['h']
        I_ext = self.neuron.I_ext
        ...
        plt.title(f'Gating Variables vs Membrane Potential (I_ext = {I_ext} μA/cm²)')
